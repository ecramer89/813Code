...
public class MathProblem
  ...
  public GPGenotype create()
    ...
    //Sub program: container that executes all statements contained within it
    //arity of the sub program is the number of statements it contains.
    //the number of void statements is arity-1. the final statement always returns the final value
    //need to use sub program construct if we want to evolve solutions that have chained commands
    Class[] types = {
    		//types defines the return type of the sub programs
        // Return type of result-producing chromosome
        CommandGene.FloatClass,
        // ADF-relevant:
        // Return type of ADF 1
        //argtypes defines the argument types of the ADFs
        CommandGene.FloatClass};
    ...
    CommandGene[][] nodeSets = { {
    	...
    	//need to define a Variable that can be the named input to the functions.
    	//the third parameter indicates the type of the variable
    	vx = Variable.create(conf, "X", CommandGene.FloatClass),
    			
    			//nodes can be functions or terminals.
    			//note that terminals (see bottom) do not have any input parameters
    			//functions always have at least one input parameter
